	...
	<div class="layui-col-md12">
		<div class="layui-card">
			...
			<div class="layui-card-body">
				<form action="RydjServlet?action=rydjListLike" method="post">
               ...
               <div style="text-align: center">
					<div class="layui-btn-group">
						<a class="layui-btn layui-btn-primary"
							href="RydjServlet?action=studentList&p=1">首页</a>
						<%--        判断是否有上一页--%>
						<c:if test="${cp>1}">
							<a class="layui-btn layui-btn-primary"
								href="RydjServlet?action=studentList&p=${cp-1}">上一页</a>
						</c:if>
						<%--        循环显示页码--%>
						<c:forEach begin="${cp-2>1 ? (cp-2) :1}"
							...
							<c:if test="${cp==e}">
								<a class="layui-btn layui-btn-danger"
									href="RydjServlet?action=studentList&p=${e}">${e}</a>
							</c:if>
							<c:if test="${cp!=e}">
								<a class="layui-btn layui-btn-primary"
									href="RydjServlet?action=studentList&p=${e}">${e}</a>
						...
						<c:if test="${cp<tp}">
							<a class="layui-btn  layui-btn-primary"
								href="RydjServlet?action=studentList&p=${cp+1}">下一页</a>
						</c:if>
						<a class="layui-btn  layui-btn-primary"
							href="RydjServlet?action=studentList&p=${tp}">尾页</a> 
					</div>
				</div>
