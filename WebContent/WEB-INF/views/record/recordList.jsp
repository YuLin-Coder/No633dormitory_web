...
<body>
	...
	<div class="layui-col-md12">
		<div class="layui-card">
			<div class="layui-card-header">缺勤记录列表</div>
			<div class="layui-card-body">
			     <form action="RecordServlet?action=recordListLike" method="post">
						      <c:if test="${ flag != '2' }">
						         <div class="layui-form-item">
									    <div class="layui-inline">
									      <div class="layui-input-inline">
									       		<input type="text" placeholder="根据学生姓名" name="sname" id ="sname" class="layui-input"  />
									      </div>
									    </div>
									    <div class="layui-inline">
					...
					<tbody>
						...
						<c:forEach items="${list}" var="u">
							...
							<c:if test="${flag != '2'}">
								...
								<a href="RecordServlet?action=toUpdateRecord&id=${u.id}" 
				...
				<div style="text-align: center">
					<div class="layui-btn-group">
						<a class="layui-btn layui-btn-primary"
							href="RecordServlet?action=recordList&p=1">首页</a>
						<%--        判断是否有上一页--%>
						<c:if test="${cp>1}">
							<a class="layui-btn layui-btn-primary"
								href="RecordServlet?action=recordList&p=${cp-1}">上一页</a>
						</c:if>
						<%--        循环显示页码--%>
						<c:forEach begin="${cp-2>1 ? (cp-2) :1}"
							...
							<c:if test="${cp==e}">
								<a class="layui-btn layui-btn-danger"
									href="RecordServlet?action=recordList&p=${e}">${e}</a>
							</c:if>
							<c:if test="${cp!=e}">
								<a class="layui-btn layui-btn-primary"
									href="RecordServlet?action=recordList&p=${e}">${e}</a>
						...
						<c:if test="${cp<tp}">
							<a class="layui-btn  layui-btn-primary"
								href="RecordServlet?action=recordList&p=${cp+1}">下一页</a>
						</c:if>
						<a class="layui-btn  layui-btn-primary"
							href="RecordServlet?action=recordList&p=${tp}">尾页</a> 
					</div>
				</div>
	...
	var form = layui.form,
	 ...
	 $('a.layui-btn-danger').click(function(e){
	   		            var id = $(e.currentTarget).parent().find('.weui-input').attr('id');
				   		if(confirm("确定要删除吗？")){
				   		  $.post("RecordServlet?action=deleteRecord&id="+id, function(data) {
